...
function TrackVehiclePanel({ shopEmail }) {
  ...
  const pendingEstimates = (data?.estimates || []).filter(e => e.status === "sent" || e.status === "draft");
  const today = new Date(); today.setHours(0,0,0,0);
  const upcomingAppointments = (data?.appointments || [])
    .filter(a => a.status !== "cancelled" && a.status !== "completed" && new Date(a.date) >= today)
        ...
        {step === "dashboard" && customer && (
          <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard icon={FileText} label="Pending estimates" value={pendingEstimates.length} />
              <StatCard icon={CalendarClock} label="Next appointment" value={
                nextAppointment ? new Date(nextAppointment.date).toLocaleDateString(undefined, { month: "short", day: "numeric" }) : "None"
              } />
            </div>
            ...
            {pendingEstimates.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-white">Estimates awaiting your review</span>
                </div>
                <div className="space-y-2">
                  {pendingEstimates.slice(0, 5).map((e) => (
                    <div key={e.id} className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
                      <div className="min-w-0">
                        <p className="text-white text-sm font-medium truncate">{e.vehicle_info || "Vehicle"}</p>
                        <p className="text-zinc-500 text-xs truncate">{e.service_reason || e.estimate_number || "—"}</p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-white text-sm font-semibold">${(e.total || 0).toFixed(2)}</span>
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${estStatusColors[e.status] || "bg-zinc-500/20 text-zinc-300"}`}>
                          {e.status}
                        </span>
                      </div>
                    </div>
            ...
            {nextAppointment && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${apptStatusColors[nextAppointment.status] || "bg-zinc-500/20 text-zinc-300"}`}>
                    {nextAppointment.status}
                  </span>
                </div>
                <p className="text-white font-medium">
                  {new Date(nextAppointment.date).toLocaleDateString(undefined, { weekday: "long", month: "short", day: "numeric" })}
                  {nextAppointment.time_slot ? ` · ${nextAppointment.time_slot}` : ""}
                </p>
                <p className="text-zinc-400 text-sm mt-1">{nextAppointment.service_type || nextAppointment.vehicle_info || "—"}</p>
              </div>
            )}
