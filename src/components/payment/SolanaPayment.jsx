...
const SOL_PRICE_USD = 19.99;

export default function SolanaPayment({ userEmail, onClose }) {
  ...
  React.useEffect(() => {
    ...
    const fetchSolPrice = async () => {
      try {
        ...
        const amount = (SOL_PRICE_USD / solPrice).toFixed(4);
        setSolAmount(amount);
      } catch (error) {
  ...
  const handleVerify = async () => {
    ...
    setVerifying(true);
    try {
      const result = await base44.functions.invoke('verifySolanaPayment', {
        signature: transactionSignature,
        email: userEmail
      });

      if (result.data.success) {
        alert('✅ Payment verified! Premium access activated.');
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-900 rounded-2xl border border-white/10 max-w-lg w-full p-6 space-y-6">
        <div className="text-center">
          ...
          <p className="text-zinc-400 text-sm">Send SOL to activate Premium access</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
            ...
            <ol className="text-zinc-300 text-xs space-y-1 list-decimal list-inside">
              ...
              <li>Click "Verify Payment" to activate Premium</li>
            </ol>
          </div>
